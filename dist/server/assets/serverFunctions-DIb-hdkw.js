...
import { p as products, f as formatPrice, s as shippingRegions, a as paymentMethods } from "./shipping-DGz5nmQT.js";
...
async function getDashboardMetrics(ctx, range) {
  ...
  const products2 = productsRes.data ?? [];
  let saleItems = [];
  if (sales.length > 0) {
  ...
  const inventoryValue = products2.reduce((sum, p) => sum + Number(p.stock_quantity) * Number(p.cost_price), 0);
  const lowStockItems = products2.filter((p) => p.stock_quantity <= p.reorder_level).map((p) => ({ id: p.id, name: p.name, stock_quantity: p.stock_quantity, reorder_level: p.reorder_level }));
