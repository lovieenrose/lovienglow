...
function applyDisplayOverrides(meta, realStock) {
  ...
  switch (meta.statusOverride) {
    ...
    default:
      return realStock > 0 ? { stock: realStock, purchasable: true } : { stock: 0, purchasable: false, badge: "out_of_stock" };
  }
}
async function listPublicCatalog() {
  const supabase = getSupabaseAdmin();
  const [{ data: products, error: productsError }, { data: sets, error: setsError }] = await Promise.all([
    supabase.from("products").select("id, name, description, selling_price, stock_quantity, image_url, storefront_meta").eq("owner_id", STOREFRONT_OWNER_ID).not("storefront_meta", "is", null),
